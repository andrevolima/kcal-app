...
O onboarding do atleta pertence ao domínio `athletes`: emissão autenticada, convite temporário com hash persistido e ativação pública. A ativação apenas define a primeira senha; autenticação posterior continua no domínio `accounts` via JWT.
